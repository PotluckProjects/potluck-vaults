...
const fetchTokens = async () => {
  ...
  try {
    const response = await axios.get(`https://newapi.potluckprotocol.com/prices?_=${cacheBuster}`);
...
const fetchLPs = async () => {
  ...
  try {
    const response = await axios.get(`https://newapi.potluckprotocol.com/lps?_=${cacheBuster}`);
    return response.data;
  } catch (err) {
    console.error(err);
    return {};
